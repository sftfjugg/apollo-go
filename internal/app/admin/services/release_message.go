...
func (s releaseMessageService) Create(appId, clusterName, comment, name, namespaceId, operator string, isPublic bool, keys []string) error {
	...
	if isPublic {
		return s.CreatePublicGlobal(release, namespaceId, keys)
	} else {
...
//公共配置发布（全局配置）
func (s releaseMessageService) CreatePublicGlobal(release *models.Release, namespaceId string, keys []string) error {

	appNamespaces, err := s.appNamespaceRepository.FindAppNamespaceByPublic(release.ClusterName)
	if err != nil {
		return errors.Wrap(err, "call appNamespaceRepository.FindClusterNameByAppId() error")
	}
	//通知所有泳道的数据库模型
	...
	for i := range appNamespaces {
		releaseMessage := new(models.ReleaseMessage)
		releaseMessage.Message = appNamespaces[i].AppId + "+" + appNamespaces[i].ClusterName + "+" + "application"
		messaages = append(messaages, releaseMessage.Message)
		releaseMessages = append(releaseMessages, releaseMessage)
	}
	db := s.db.Begin()
